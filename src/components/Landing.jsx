...
function Landing() {
    return (
        <section id='AboutMe' className='shrink-0 pb-20 h-full flex flex-col gap-4 justify-around items-center px-4 md:px-2 snap-start'>
            <article className='shrink-0 flex flex-col justify-center items-center'>
                <header className='flex flex-col'>
                    <h2 className='font-mono text-base sm:text-2xl lg:text-3xl'>Hi there! My name is</h2>
                    <h1 className='font-me text-4xl sm:text-5xl md:text-6xl lg:text-8xl font-bold'>Manish Ambre</h1>
                </header>
                <section className='shrink-0 flex flex-row gap-4 items-start justify-center'>
            ...
            <FaArrowDown className='shrink-0 text-3xl go' />
        </section>
    );
}
